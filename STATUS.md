...
> 最後更新：2026-08-23（課程大分類與分類可見權限正在正式發布）。

---

## 🗂️ 2026-08-23：課程大分類與分類層級權限（🚀 正式發布中）
...
四種身份與所有下游資料表的直接讀取，共同限制與既有課程回填皆通過。針對性 ESLint、35/35 單元測試、
production build、`git diff --check` 通過；Impeccable 掃描無問題。真瀏覽器以 1440px 與 390px 驗證
大分類首頁及分類內課程頁，手機水平溢出為 0、瀏覽器錯誤為 0。

**正式環境邊界**：沒有寫入正式 Supabase、沒有 push／部署。上線必須先套
`20260823094617_add_course_categories.sql`，再部署前端；若反向部署，前端會先查詢尚不存在的
`course_categories`／`courses.category_id`。正式套 SQL 後應先在後台調整預設分類名稱與課程歸屬，
再依實際培訓安排新增或發佈其他大分類。
